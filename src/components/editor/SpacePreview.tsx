...
const PREVIEW_COLOR = '#22c55e'; // Green for spaces
const POINT_COLOR = '#16a34a'; // Darker green
const CLOSE_COLOR = '#ef4444'; // Red (when about to close)

/**
 * Visual preview during space polygon drawing
 * Shows placed points, connecting lines, and preview line to cursor
 */
export function SpacePreview() {
  const { activeTool, spacePlacement, distanceInput } = useToolStore();
  const storeyElevation = useStoreyElevation();

  const { points, previewPoint } = spacePlacement;
  ...
  // Don't show if not in space-draw mode or no points (after all hooks)
  ...
  return (
    <group>
      ...
      {points.length >= 3 && (
        <SpaceFillPreview points={points} storeyElevation={storeyElevation} />
      )}
      ...
      {points.length > 0 && previewPoint && (() => {
        ...
        return (
          ...
          >
            <div
              style={{
                background: distanceInput.active ? '#22c55e' : 'rgba(0, 0, 0, 0.75)',
                color: 'white',
                padding: '2px 6px',
                borderRadius: '4px',
                fontSize: '12px',
                ...
                whiteSpace: 'nowrap',
                border: distanceInput.active ? '1px solid #4ade80' : 'none',
              }}
            >
...
/**
 * Semi-transparent fill for the space preview
 */
function SpaceFillPreview({ points, storeyElevation }: { points: { x: number; y: number }[]; storeyElevation: number }) {
